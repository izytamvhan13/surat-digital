...
@section('subtitle', 'Perbarui detail surat.')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="perihal" id="perihal" required
                        ...
                        @foreach($kategoriSurat as $kategori)
                            <option value="{{ $kategori->nama }}" {{ old('perihal', $surat->perihal) == $kategori->nama ? 'selected' : '' }}>
                                {{ $kategori->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="isi_singkat" id="isi_singkat" rows="4" 
                              class="w-full px-4 py-2 border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500">{{ old('isi_singkat', $surat->isi_singkat) }}</textarea>
                    @error('isi_singkat')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    <label for="file_surat" class="block text-sm font-medium text-slate-700 mb-2">File Surat (Kosongkan jika tidak ingin diubah)</label>
                    ...
                    @if($surat->file_surat)
                    <p class="text-xs text-slate-500 mt-2">File saat ini: <a href="{{ url('storage/' . $surat->file_surat) }}" target="_blank" class="text-sky-600 hover:underline">{{ Str::after($surat->file_surat, 'surat_masuk/') }}</a></p>
                    @endif
                    @error('file_surat')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
